...
### `entityCohesionScore_PerEntityMean` (double)
- **What**: Mean of per-entity cohesion scores
- **When**: Computed after all mentions are processed
- **How**:
$$
\text{entityCohesionScore}_{\text{mean}} = \frac{1}{N}\sum_{i=1}^{N} \left( \frac{r_i \times \max(a_{i,\text{EOS}}, a_{i,\text{EODoc}}) }{1 + d_i} \right)
$$
  where $N$ = uniqueEntityCount, $r_i$ = rementions of entity $i$, $d_i$ = avgSentenceDistance for entity $i$
- **Interpretation**: Higher = entities are consistently re-used with proximity and maintained activation

### `entityCohesionScore_PerEntityMedian` (double)
- **What**: Median of per-entity cohesion scores
- **When**: Computed after all mentions are processed
- **How**:
$$
\text{entityCohesionScore}_{\text{median}} = \text{median}\left\{\frac{ r_i \times \max(a_{i,\text{EOS}}, a_{i,\text{EODoc}}) }{1 + d_i} \right\}_{i=1}^{N}
$$
- **Interpretation**: Represents "typical" entity cohesion, less affected by highly coherent or incoherent outliers

### `topicPersistenceScore_PerEntityMean` (double)

- **What**: Mean of per-entity topic persistence scores.  
- **When**: Computed after all mentions are processed.  
- **How**:  

The mean topic persistence score is defined as:

$$
\text{topicPersistenceScore}_{\text{mean}} = \frac{1}{N} \sum_{i=1}^{N} \Big( p_i \cdot (1 - \text{decay}_i) \cdot c_i \Big)
$$
...
p_i = 
\begin{cases} 
1 & \text{if entity $i$ appears in multiple paragraphs} \\
0 & \text{otherwise} 
\end{cases}
$  
- Entity-specific decay rate:  
$\large
\text{decay}_i = \frac{a_{i,\text{EOS}} - a_{i,\text{EODoc}}}{a_{i,\text{EOS}}}
$
- Entity coverage:  
$\large
c_i = \frac{\text{mentions of entity } i}{\text{total sentences}}
$ 

- **Interpretation**: Higher values indicate that entities maintain a consistent presence and activation throughout the document.


### `topicPersistenceScore_PerEntityMedian` (double)
- **What**: Median of per-entity topic persistence scores
- **When**: Computed after all mentions are processed
- **How**:
$$
\text{topicPersistenceScore}_{\text{median}} = \text{median}\left\{ p_i \times (1 - \text{decay}_i) \times c_i \right\}_{i=1}^{N}
$$
- **Interpretation**: Typical topic persistence behavior across entities

### `localCoherenceScore_PerEntityMean` (double)
- **What**: Mean of per-entity local coherence scores
- **When**: Computed after all mentions are processed
- **How**:
$$
\text{localCoherenceScore}_{\text{mean}} = \frac{1}{N}\sum_{i=1}^{N} \left( \frac{\text{density}_i \times \text{consecutive}_i}{d_i} \right)
$$
  where:
  - $\large d_i$ = avgSentenceDistance for entity $i$
  - $\large \text{density}_i = \frac{\text{mentions of entity } i}{\text{totalSentences}}$
    
  - $\large \text{consecutive}_i = \frac{\text{mentions in consecutive sentences}}{\text{totalSentences}}$

- **Interpretation**: Higher = entities appear in close proximity with frequent consecutive mentions

### `localCoherenceScore_PerEntityMedian` (double)
- **What**: Median of per-entity local coherence scores
- **When**: Computed after all mentions are processed
- **How**:
$$
\text{localCoherenceScore}_{\text{median}} = \text{median}\left\{ \frac{\text{density}_i \times \text{consecutive}_i}{d_i} \right\}_{i=1}^{N}
$$
- **Interpretation**: Typical local coherence pattern across entities

### `globalCoherenceScore_PerEntityMean` (double)
- **What**: Mean of per-entity global coherence scores
- **When**: Computed after all mentions are processed
- **How**:
$$
\text{globalCoherenceScore}_{\text{mean}} = \frac{1}{N}\sum_{i=1}^{N} \left( a_i \times \text{paragraphCoverage}_i \times p_i \right)
$$
  where:
  - $\large a_i = \max(a_{i,\text{EOP}}, a_{i,\text{EODoc}})$ with fallback to $a_{i,\text{EOS}}$
    
  - $\large \text{paragraphCoverage}_i = \frac{\text{unique paragraphs with entity } i}{\text{totalParagraphs}}$
  - $\large p_i = 1$ if entity $i$ appears in 2+ paragraphs, else $0$
- **Interpretation**: Higher = entities maintain strong activation and presence across document structure

### `globalCoherenceScore_PerEntityMedian` (double)
- **What**: Median of per-entity global coherence scores
- **When**: Computed after all mentions are processed
- **How**:
$$
\text{globalCoherenceScore}_{\text{median}} = \text{median}\left\{ a_i \times \text{paragraphCoverage}_i \times p_i \right\}_{i=1}^{N}
$$
- **Interpretation**: Typical global coherence pattern across entities
